...
int main()
{
  ...
  // Charset
  // otfinfo -u AaKaiSong2WanZi2_remapped.ttf | perl -pe 's/^uni([0-9A-F]+) .*$/\1/g' > AaKaiSong2WanZi2.charset.txt
  // ttx AaKaiSong2WanZi2_remapped.ttf -t cmap -o - | perl -ne 'if (/<cmap_format_4 platformID="0" platEncID="3"/ .. /<\/cmap_format_4/) { printf("%04X\n", hex($1)) if /code="0x([0-9a-f]+)"/i; }' > AaKaiSong2WanZi2.charset.txt

...
int main()
{
  ...
  // Filter/Global CDT Configuration Register
  buf[0] = 0xf4;
  ...
  */
    ...
    #define N_SAMPLES 16
    uint16_t values[N_SAMPLES];
    for (int i = 0; i < N_SAMPLES; i++) {
      HAL_I2C_Mem_Read(&i2c1, 0x5A << 1, 0x04, I2C_MEMADD_SIZE_8BIT, buf, 1, 1000);
      HAL_I2C_Mem_Read(&i2c1, 0x5A << 1, 0x05, I2C_MEMADD_SIZE_8BIT, buf + 1, 1, 1000);
      values[i] = ((uint16_t)buf[1] << 8) | buf[0];
      HAL_Delay(5);
    }
    for (int i = 0; i < N_SAMPLES; i++)
    ...
    uint16_t value = values[N_SAMPLES / 2];
    if (count < 8 && value <= 420) count = (count + 2 > 8 ? 8 : count + 2);
    else if (count > 0) count--;
    char s[64];
    int n = snprintf(s, sizeof s, "value = %4d, %s\r\n",
      value,
      // value <= 420 ? "++++++" : "");
      count >= 5 ? "++++++" : "");
      // "");

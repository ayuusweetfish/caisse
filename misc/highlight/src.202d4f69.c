...
volatile bool i2c_dma_busy = false;
...
#define N_ELECTRODES 6
  ...
  bool screen_present = (ready == HAL_OK);
  ...
  while (1) {
    ...
    if (screen_present && T % 20 == 0) {
      ...
      char s[32];
      if (error) {
        snprintf(s, sizeof s, "Error code\n0x%08lx", i2c1.ErrorCode);
        print_string(0, 0, s);
      } else {
        for (int i = 0; i < N_ELECTRODES; i++) {
          snprintf(s, sizeof s, "%c%d:%4ld",
            d[i].on ? '*' : ' ',
            i,
            ((uint32_t)buf[i * 2 + 1] << 8) | buf[i * 2 + 0]
          );
          print_string(i % 4, (i / 4) * 8, s);
        }
      }
      i2c_dma_busy = true;
      HAL_I2C_Master_Transmit_DMA(&i2c1, 0x3C << 1, dispcmdbuf, sizeof dispcmdbuf);
    }
    if (T == 100) T = 0;

    HAL_Delay(10);

    while (i2c_dma_busy) HAL_Delay(1);
  }
}
